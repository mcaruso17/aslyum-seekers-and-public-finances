...
*eventdd step 3 - SAI and non-SAI
...
eventdd ln_q4_`x' cas ln_q2t1tot ln_q2t2tot ln_q2t3tot population african_pop foreign_pop ln_employees_n ln_net_taxbase if psm == 3, timevar(t) method(hdfe, absorb(i.code i.year)) lags(3) leads(3) accum over(_treated) jitter(0.2) graph_op(scheme(s1mono) ytitle("`titlegraph' (log)") legend(pos(6) order(2 "Without SAI" 5 "With SAI" 1 "95% CI") rows(1)) ylabel(, format("%04.2f"))) coef_op(g1(ms(Sh)) g2(ms(Oh)))

graph export "$folder/images /eventdd_ln_q4_`x'_step3.jpg", as(jpg) name("Graph") quality(90) replace 
	
}


eventdd ln_q4tot cas ln_q2t1tot ln_q2t2tot ln_q2t3tot population african_pop foreign_pop ln_employees_n ln_net_taxbase if psm == 3, timevar(t) method(hdfe, absorb(i.code i.year)) lags(3) leads(3) accum over(_treated) jitter(0.2) graph_op(scheme(s1mono) ytitle("Total spending (log)") legend(pos(6) order(2 "Without SAI" 5 "With SAI" 1 "95% CI") rows(1)) ylabel(, format("%04.2f"))) coef_op(g1(ms(Sh)) g2(ms(Oh)))

graph export "$folder/images /eventdd_ln_q4tot_step3.jpg", as(jpg) name("Graph") quality(90) replace 
