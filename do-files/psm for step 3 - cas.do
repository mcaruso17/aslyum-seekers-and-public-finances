...
save data_mc_cas, replace

sort code year
bysort code: keep if (days_difference_cas < 0 & mayor_change2 == 1) & (days_difference_cas[_n-1] > 0)
duplicates drop code, force


gen psm_treated = 1
save psm_treated3cas, replace

use data_mc_cas, clear
keep if year == 2017
duplicates drop code, force
merge 1:1 code using psm_treated3cas, gen(psm)
...
save psm_treated_control3cas, replace

use data_mc_cas, clear
keep if year == 2002
duplicates drop code, force
merge 1:1 code using psm_treated_control3cas, gen(psm)

save psm_treated_control3_2002cas, replace
...
merge m:1 code using data_psm3cas, gen(psm)

save datapsm3cas, replace
