...
bysort code: keep if year == sai_opening_year - 1 | year == cas_opening_year - 1


gen psm_treated_asrc = 1
save psm_treated_asrc, replace

use data0223_step2, clear
keep if year == 2017
duplicates drop code, force
merge 1:1 code using psm_treated_asrc, gen(psm)

drop if psm==2
replace psm_treated_asrc = 0 if psm_treated_asrc ==.
drop year

keep code name region_code surf mc altitude altitude_min latitude longitude zone urban climate psm_treated_asrc prov_code population politics foreign_pop q2t1tot q2t2tot q2t3tot
save psm_treated_control_asrc, replace

use data0223_step2, clear
keep if year == 2002
duplicates drop code, force
merge 1:1 code using psm_treated_control_asrc, gen(psm)
...
order year code name mayor_change2 rep_name
